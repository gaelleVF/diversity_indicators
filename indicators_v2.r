...
create_scenar <- function(rep_data, ssr_data, departement, year, type_scenar, type_var, prop){
  ## Fonction crÈant les scÈnarios
  # rep_data = donnÈes de rÈpartition des variÈtÈs dans les dÈpartements
  # departement = departement depuis lequel on veut crÈer le scÈnario
  # year = annÈe depuis laquelle on veut crÈer le scÈnario
  # type_scenar = 
      # "random" = remplace alÈatoirement les varietes du paysage
      # "domin" = remplacer les variÈtÈs dominantes (top 5 ?)
      # "mino" = remplacer les variÈtÈs minoritaires
  # type_var = "OL","L","PV"
  # prop = proportion de remplacement. Vector dont la longueur est Ègal au nombre d'annÈes d'Èvolution du scÈnario
  

  d = rep_data[rep_data$departement %in% departement & rep_data$annee %in% year,]
  variete = unique(d$variete)
  # vÈrif proportion de variÈtÈs manquantes au ssr
  
  
  
  
  # si type_var == "mÈlange", crÈer des mÈlanges ‡ partir des variÈtÈs dominantes ? Des mÈlanges ‡ combien ? 
  # Faire par exemple 2-3 mÈlanges diffÈrents et les tirer alÈatoirement
  # donner une annÈe de dÈpart et puis remplacer les variÈtÈs d'annÈe en annÈe dans les variÈtÈs existantes
  
  # choix des dÈpartements = ceux contrastÈs pour lesquels on a des donnÈes pour beaucoup de variÈtÈs
...
# Check if same number of varieties and proportions for each scenario
    ...
    d <- which(a != b)
    if(length(d)>0){stop("Attention : le nombre de vari√©t√© et de proportions pour le(s) sc√©nario(s) ",d," ne concordent pas.",sep="")}
...
# Check if varieties' names are in ssr_data, give proportion of missing data
    ...
    if(length(prop)>0){warning(paste("Attention : la proportion de l'assolement pour laquelle il manque des donn√©es ssr pour le(s) scenario(s) ",paste(b,collapse=" et "),
